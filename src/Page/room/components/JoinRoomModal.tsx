...
export const JoinRoomModal = ({ isOpen, onClose }: Props) => {
    ...
    const current = 1; // thay đổi khi cần theo state

    return (
        <Modal isOpen={isOpen} onClose={onClose} title="THAM GIA VÀO NHÓM">
            <div className="w-[980px] max-w-full">
                <div className="flex gap-6">
                    {/* Left: Public list */}
                    <div className="flex-1 bg-[#0ec0db] rounded-lg p-6">
                        <div className="mb-4">
                            <h3 className="text-3xl font-semibold text-white text-center">Công khai</h3>
                        </div>
                            ...
                            {ROOMS.map((r) => (
                                ...
                                >
                                    ...
                                    <div className="w-full flex justify-between items-center px-2">
                                        <span className="text-sm font-medium text-slate-800 text-start flex-1">{r.name}</span>
                                ...
                                typeof p === "number" ? (
                                    <button
                                        ...
                                        className={`w-8 h-8 flex items-center justify-center rounded-md text-sm transition
                                            ${p === current ? "bg-white text-[#0C1A57] font-medium" : "bg-white/20 text-[#0C1A57] hover:bg-white/40"}`}
                                    >
                                        {p}
                                    </button>
                                ) : (
                    ...
                    {/* Right column: Create + Find */}
                    <div className="w-80 flex flex-col gap-6">
                        {/* Create room */}
                        <div className="bg-[#13a8c7] rounded-lg p-5">
                            <h4 className="text-2xl font-semibold text-white text-center mb-3">Tạo phòng</h4>
                            <div className="flex gap-3 justify-center">
                                <button className="px-4 py-2 bg-white/80 text-[#0C1A57] rounded-full font-medium hover:bg-white">
                                    Riêng tư
                                </button>
                                <button className="px-4 py-2 bg-white/80 text-[#0C1A57] rounded-full font-medium hover:bg-white">
                                    Công khai
                        ...
                        {/* Find room */}
                        <div className="bg-[#13a8c7] rounded-lg p-5">
                            <h4 className="text-2xl font-semibold text-white text-center mb-3">Tìm phòng</h4>
                            <div className="flex gap-3 items-center">
                                ...
                                />
                                <button className="px-4 py-2 bg-pink-300 text-[#0C1A57] rounded font-medium hover:bg-pink-400">
                                    Nhập
                                </button>
                            </div>
                        </div>

                        {/* optional helper / info */}
                        <div className="text-center text-white/90 text-sm">
                            Chọn phòng công khai để tham gia, hoặc tạo phòng mới.
                        </div>
                    </div>
                </div>
            </div>
