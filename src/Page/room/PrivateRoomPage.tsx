...
const PrivateRoomPage = () => {
  ...
  const [currentRoomStatus, setCurrentRoomStatus] = useState<RoomStatus | null>(
    null
  );
    ...
    if (action === "play") {
      if (trackType === "file" && audio) {
        audio
          ...
          .catch((err) =>
            console.warn("Autoplay blocked for audio file:", err)
          );
      } else if (
        trackType === "youtube" &&
        youtubePlayer &&
      ...
      if (trackType === "file" && audio) {
        ...
        if (
          typeof youtubePlayer.getPlayerState === "function" &&
          youtubePlayer.getPlayerState() !== window.YT.PlayerState.PAUSED
        ) {
    ...
    youtubePlayer = new window.YT.Player("youtube-player", {
      ...
      events: {
        onReady: (event: any) => {
          ...
          if (
            roomStatusRef.current === RoomStatus.ON_WORKING &&
            !isPausedRef.current
          ) {
    ...
    try {
      ...
      if (response.status === 200 && response.data?.result) {
        ...
        // Chuyển từ BREAK -> WORKING: TỰ ĐỘNG TIẾP TỤC
          ...
          (currentRoomStatus === RoomStatus.ON_REST ||
            currentRoomStatus === RoomStatus.ON_LONG_REST) &&
          nextRoomStatus === RoomStatus.ON_WORKING
        ) {
          if (!isUserPaused.current) {
            musicControl("play");
        ...
        // Bắt đầu chu kỳ đầu tiên
          ...
          (currentRoomStatus === null ||
            currentRoomStatus === RoomStatus.STOP) &&
          nextRoomStatus === RoomStatus.ON_WORKING
        ) {
    ...
    >
      ...
      <span className="z-10 text-xl font-semibold mb-4 text-white/90">
        ...
        {(currentRoomStatus === RoomStatus.ON_REST ||
          currentRoomStatus === RoomStatus.ON_LONG_REST) &&
          `☕ ${t("privateRoom.break")}`}
        {(!currentRoomStatus || currentRoomStatus === RoomStatus.STOP) &&
          t("privateRoom.start")}
      </span>
      ...
      <div className="z-10 flex flex-col items-center bg-white/80 px-6 py-3 rounded-lg">
        ...
        {(trackType === "file" && audio) ||
        (trackType === "youtube" && youtubePlayer) ? (
