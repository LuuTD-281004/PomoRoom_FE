...
import { getUsersRanking } from "@/axios/user";
...
const RankingPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchRanking = async () => {
      try {
        setLoading(true);
        const res = await getUsersRanking(1, 10);
        setRanking(res?.result?.data || []);
      } catch (err) {
  ...
  const others = useMemo(() => ranking.slice(3, 10), [ranking]);

  // CHỈNH: an toàn khi u là undefined
  const displayName = (u?: RankingItem | null) =>
    u
  ...
  const myLoops = useMemo(() => {
    const me = ranking.find((u) => u.id === authenticatedUser?.id);
    return me ? me.totalLoopCompleted : 0;
  }, [ranking, authenticatedUser?.id]);
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <span className="text-[#0C1A57]">Đang tải bảng xếp hạng...</span>
      </div>
    );
  }
        ...
        <section className="w-full flex justify-center bg-[#F5F9FF] py-20 px-6">
          <div className="flex items-end justify-center gap-12 max-w-6xl w-full">
            ...
            <div className="flex flex-col items-center mt-12">
              ...
              <div className="bg-[#E3F2FD] rounded-2xl shadow-lg text-center -mt-8 w-56 p-8">
                <p className="font-bold text-[#0C1A57] text-lg">
                  {top3[1] ? displayName(top3[1]) : "--"}
                </p>
                <div className="flex items-center justify-center gap-2 mt-2">
                  <Flame className="w-6 h-6 text-red-500" />
                  <p className="text-2xl font-extrabold text-green-600">
                    {top3[1]?.totalLoopCompleted ?? 0}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col items-center">
              ...
              <div className="bg-[#E3F2FD] rounded-2xl shadow-lg text-center -mt-10 w-64 p-10">
                <p className="font-bold text-[#0C1A57] text-xl">
                  {top3[0] ? displayName(top3[0]) : "--"}
                </p>
                <div className="flex items-center justify-center gap-2 mt-3">
                  <Flame className="w-7 h-7 text-red-500" />
                  <p className="text-3xl font-extrabold text-green-600">
                    {top3[0]?.totalLoopCompleted ?? 0}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col items-center mt-16">
              ...
              <div className="bg-[#E3F2FD] rounded-2xl shadow-lg text-center -mt-8 w-56 p-8">
                <p className="font-bold text-[#0C1A57] text-lg">
                  {top3[2] ? displayName(top3[2]) : "--"}
                </p>
                <div className="flex items-center justify-center gap-2 mt-2">
                  <Flame className="w-6 h-6 text-red-500" />
                  <p className="text-2xl font-extrabold text-green-600">
                    {top3[2]?.totalLoopCompleted ?? 0}
                  </p>
                </div>
              </div>
            </div>
            ...
            {others.map((user, idx) => {
              ...
              return (
                ...
                >
                  ...
                  <p className="text-green-600 font-bold mr-16">
                    {user.totalLoopCompleted}
                  </p>
                </div>
              );
            })}
              ...
              <div className="bg-[#EAF6FF] rounded-md py-10 px-12 border-t-8 border-[#567CB1]">
                ...
                <div className="flex gap-16 items-start justify-center">
                  <div className="text-center">
                    <div className="text-6xl font-extrabold text-[#0C1A57]">
                      {myLoops}
                    </div>
